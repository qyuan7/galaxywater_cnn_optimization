...
def training(nets,names ,n_grid=32, build_r=32., build_prefixs = [],dbloss=False,batchsize=4):
    for idx, net in enumerate(nets):
        ...
        log = open('%s/cnn_net_res10lr_32c_from0_new_v%d.log'%(name,idx),'a')
        env_train = {'idxs':trainidxs, 'net':net, 'device':device,'dbloss':dbloss,
                     'optimizer':optimizer, 'log':log,'n_grid':n_grid,'build_r':build_r, 'scheduler':scheduler}
        
        env_test = {'idxs':testidxs, 'net':net, 'device':device,'dbloss':dbloss,
                     'optimizer':optimizer, 'log':log,'n_grid':n_grid,'build_r':build_r, 'scheduler':scheduler}
        
        for epoch in range(start_epoch, start_epoch+1000):  # loop over the dataset multiple times
...
if __name__ == '__main__':
    ...
    with open("validation.txt") as f2:
        data2 = f2.readlines()
    trainidxs = [d.strip() for d in data1]
    #trainidxs  = []
    testidxs   = [d.strip() for d in data2]
    print(len(testidxs))
